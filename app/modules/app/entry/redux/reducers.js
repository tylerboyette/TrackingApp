...
function entryReducer(state = initalState, action) {
  switch (action.type) {
    ...
    case CONSTANTS.ENTRY_LIST_SUCCESS:
      return state
        .setIn(['entries', 'list'], fromJS(action.data))
    ...
    case CONSTANTS.ENTRY_REPORT_SUCCESS:
      return state
        .setIn(['report', 'list'], fromJS(action.data))
    ...
    case CONSTANTS.ENTRY_DELETE_SUCCESS: {
      ...
      const filteredList = entryList.filter(
        entry => entry.get('_id') !== action.id,
      );
      return state
        .setIn(['entries', 'list'], fromJS(filteredList))
        .setIn(['entries', 'loading'], false)
    ...
    case CONSTANTS.ENTRY_LOAD_SUCCESS:
      return state
        .setIn(['entry', 'data'], fromJS(action.data))
