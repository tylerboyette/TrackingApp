  ...
  renderEntries = entries => {
    ...
    return entries.slice((page - 1) * pageSize, page * pageSize).map(entry => (
      <Table.Row key={entry.get('_id')}>
        ...
        <Table.Cell>{entry.get('distance')}km</Table.Cell>
        <Table.Cell>{entry.get('duration')}mins</Table.Cell>
        <Table.Cell>
          {entry.get('duration')
            ? ((entry.get('distance') / entry.get('duration')) * 60).toFixed(2)
            : entry.get('duration')}
          km/hr
        </Table.Cell>
        <Table.Cell>
          ...
          <Button
            ...
            icon="minus"
            labelPosition="left"
            onClick={this.onRemove(entry.get('_id'))}
          />
        </Table.Cell>
      </Table.Row>
    ));
