...
class ReportPage extends Component {
  ...
  renderEntries = () => {
    ...
    if (!entries.length) {
      return (
        <Table.Row>
          <Table.Cell colSpan="3">No Entries</Table.Cell>
        </Table.Row>
      );
    }
    ...
    return entries
      ...
      .map((entry, index) => (
        <Table.Row key={`report_${index}`}>
          <Table.Cell>
            {entry._id.year} - Wk {entry._id.week}
          </Table.Cell>
          <Table.Cell>
            {(entry.totalDistance / entry.count).toFixed(2)}km
          </Table.Cell>
          <Table.Cell>
            {entry.totalDuration
              ? ((entry.totalDistance / entry.totalDuration) * 60).toFixed(2)
              : 0}
            km/hr
          </Table.Cell>
        </Table.Row>
